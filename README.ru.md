...
![Платформа](https://img.shields.io/badge/platform-Windows%20x64%20%7C%20ARM64%20%7C%20x86-0078d4)
...
![Интерфейс](https://img.shields.io/badge/UI-AR%20%7C%20EN%20%7C%20ES--ES%20%7C%20FA%20%7C%20JA%20%7C%20KO%20%7C%20PT--BR%20%7C%20RU%20%7C%20TR%20%7C%20ZH--CN%20%7C%20ZH--TW-lightgrey)
...
Один portable-файл `WinStoreRegion.exe`, около 2 МБ, выпускается для x64,
...
называет, — поле поэтому и подписано «список Windows»: на английском или
китайском интерфейсе регионы останутся ровно такими, как их называет Windows.
Снимок сделан на русской Windows при масштабе 125 %.
...
- **Сборки ARM64 и 32-битная x86 на живых устройствах.** Все три архитектуры
  собираются на каждый push и выкладываются с каждым релизом, то есть
  собираются они все. Ни одна из этих двух не запускалась на настоящем
  устройстве. Они выложены потому, что машина, способная их запустить, —
  единственный способ это изменить, а не потому, что здесь кто-то утверждает,
  что они работают.
## Требования
...
- x64, ARM64 или 32-битный x86 — каждый релиз несёт все три. На ARM64-устройстве
  работает и x64-сборка через эмуляцию Windows; этот путь хотя бы проверялся на
  x64-железе. Нативная
...
- Одиннадцать языков интерфейса: арабский, английский, испанский, персидский,
  японский, корейский, португальский, русский, турецкий и китайский в двух
  письменностях. Русский и английский написаны самим автором; остальные девять —
  машинные черновики, которые не читал ни один носитель языка, и об этом сказано
  в начале каждого файла. С арабским и персидским окно разворачивается справа
  налево, потому что так читаются эти языки. У каждого из них есть и переведённый
  README, и каждый такой файл ссылается на все остальные.
...
Язык, который читается справа налево, говорит об этом одним полем —
`direction = "rtl"`, — и окно разворачивается само: панели, подписи, кнопки,
колонки таблиц и полосы прокрутки меняют сторону. Первым был арабский, за ним
персидский — ценой одного файла и ни строчки Rust, и это здесь единственное
утверждение; иврит обойдётся тем же.
...
Ни одна из них не запускалась на настоящем устройстве своей архитектуры. Обе
всё равно выкладываются с каждым релизом: машина, способная их запустить, —
единственный способ это изменить. Они собираются — и это всё, что утверждается.
