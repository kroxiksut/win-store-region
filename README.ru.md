...
![Тесты](https://img.shields.io/badge/tests-240%20passing-brightgreen)
...
этом проекте обязаны называть своё доказательство. Каждая строка ниже опирается
на датированный прогон, записанный в диагностический лог.

| Что | Где | Когда |
|---|---|---|
| Полный цикл: запись восстановления → смена региона с обратным чтением → поиск приложения под временным регионом → установка с прогрессом → ранний возврат региона → подтверждённое завершение | Windows 10 22H2 (19045) | Проверено на двух приложениях, 21–22.08.2026 |
| Handoff: проверка подписи установщика, смена региона, запуск, возврат региона, запись в журнал | Windows 10 22H2 | Проверено, 21.08.2026 |
| Скачивание установщика Store по Product ID с проверкой подписи и подписанта | Windows 10 22H2 | Проверено на живой службе, 22.08.2026 |
| Отказ от установки продукта, который каталог не выдаёт этому устройству, **до** смены региона | Windows 10 22H2 | Проверено на продукте, у которого только Xbox-пакеты |
| Транзакция региона переживает отказ: во всех неуспешных прогонах регион возвращён, запись восстановления снята | Windows 10 22H2 | Проверено на всех неуспешных прогонах |
| Механика смены региона, обратного чтения и восстановления | Виртуальная машина с Windows 11 | Проверено ранее в разработке |

Что **не** проверено — и так и сказано:

- **Прогон на машине, отличной от машины разработчика**, после того как были
  доделаны пути через кнопки. Выделенная тестовая ВМ на текущей сборке не
  использовалась.
...
- **Нативная сборка под ARM64.** Она собирается из этих исходников без правок,
  но на живом ARM64-устройстве не запускалась и в поставку не входит.
